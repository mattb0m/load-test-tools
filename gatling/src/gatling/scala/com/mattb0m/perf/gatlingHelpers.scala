...
class HttpHeaderInjector(simulation:Simulation) {
	...
	private val runId = ConfigLoader.load (
	...
	private def addDynatraceHeader(request:Request, session:Session): Unit = {
		...
		request.getHeaders.set("x-dynaTrace", s"ID=${ID};VU=${VU};NA=${NA};SI=${SI};AN=${AN};SN=${SN};TE=${TE}")
	}
	...
	def injectHeaders(http:HttpProtocolBuilder): HttpProtocolBuilder = {
		return http.sign((request, session) => {
			for ((name,cb) <- this.callbacks) request.getHeaders.set(name, cb.call())
			if(this.injectDT) addDynatraceHeader(request, session)
		})
	}
